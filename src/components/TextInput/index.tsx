import React, { useState } from 'react';
import { StyleSheet, Text, TextInput, View } from 'react-native';
import { black70, dark_blue, primary_orange, white } from '../../assets/styles/Colors';
import { nunito400, nunito700 } from '../../assets/styles/FontFamily';

interface Props{
    title?: string,
    placeholder?: string,
    value?: string,
    isPassword?: boolean,
    theme?: any,
}
const Input: React.FC<Props> = ({
    title,
    placeholder,
    value,
    isPassword,
    theme,
}) => {
    const [show, setShow] = useState(isPassword);
    return (
        <View>
            <Text style={[styles.fontTitle, { color: theme ? black70 : '#FFFFFFB2' }]}>
                {title}
            </Text>
            <TextInput
                style={[styles.input(value || false), { backgroundColor: theme ? white : dark_blue, borderWidth: theme ? 1 : 0, borderColor: '#E0E0E0', color: theme ? black70 : white}]}
                placeholder={placeholder}
                placeholderTextColor= {theme ? black70 : "#FFFFFF80"}
                value={value}
                secureTextEntry={show}
            />
            {isPassword && <Text onPress={() => setShow(!show)} style={styles.fontShow}>Show</Text>}
        </View>
    );
};

export default Input;

const styles = StyleSheet.create<any>({
    input: value => ({
        padding: 10,
        backgroundColor: dark_blue,
        width: '100%',
        borderRadius: 6,
        fontFamily: value ? nunito700 : nunito400,
        color: '#FFFFFF',
        marginTop: 4,
        fontSize: 16,
    }),
    fontTitle: {
        fontSize: 12,
        fontFamily: nunito400,
        // color: '#FFFFFFB2',
    },
    fontShow: {
        fontSize: 14,
        fontFamily: nunito700,
        color: primary_orange,
        position: 'absolute',
        right: 10,
        top: '50%'
    },
});
